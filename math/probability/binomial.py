...
class Binomial:
    ...
    def __init__(self, data=None, n=1, p=0.5):
        ...
        if data is None:
            ...
            sample_variance = sum(map(lambda x: x ** 2, data))\
                / len(data) - sample_mean ** 2
    ...
    def pmf(self, k):
        ...
        Args:
            k (float | int): the number of successes whose
            pmf is to be determined
        Returns:
        ...
        return self.__choose(self.n, k) * self.p ** k\
            * (1 - self.p) ** (self.n - k)

    def cdf(self, k):
        """ Calculates the CDF of a given number of successes
        Args:
            k (float | int): the number of successes
            whose CDF is to be calculated
        Returns:
            p (float): the probability of successes
